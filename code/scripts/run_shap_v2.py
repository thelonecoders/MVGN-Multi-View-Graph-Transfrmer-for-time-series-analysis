#!/usr/bin/env python3
"""SHAP analysis v2 - shap 0.52 API compatible."""
...
def main():
    ...
    # 4. Wrapper - extract 'numeric' tensor from model output dict
    ...
    # Try PermutationExplainer __call__ with max_evals (shap 0.52 API)
    try:
        explainer = shap.PermutationExplainer(forward_numpy, masker=bg_flat)
        print(f"  PermutationExplainer with max_evals={args.max_evals}", flush=True)
        result = explainer(test_flat, max_evals=args.max_evals)
        sv = result.values if hasattr(result, 'values') else np.asarray(result)
        print(f"  ✓ Got values via __call__()", flush=True)
    except Exception as e:
        print(f"  PermutationExplainer __call__ failed: {e}", flush=True)
        # Try shap_values() without max_evals
        try:
            ...
            if isinstance(sv, list):
                ...
                if sv.ndim > 3:
                    sv = sv.sum(axis=0)
            print(f"  ✓ Got values via shap_values()", flush=True)
        except Exception as e2:
            print(f"  shap_values() failed: {e2}", flush=True)
            # Fallback to KernelExplainer
            try:
                explainer = shap.KernelExplainer(forward_numpy, bg_flat)
                explainer_type = "kernel"
                print(f"  Fallback: KernelExplainer (nsamples=50)", flush=True)
                sv = explainer.shap_values(test_flat, nsamples=50)
                if isinstance(sv, list):
                    sv = np.array(sv)
                    if sv.ndim > 3:
                        sv = sv.sum(axis=0)
            except Exception as e3:
                print(f"  KernelExplainer failed: {e3}", flush=True)
                return 1
    ...
    if sv.ndim == 4:
        ...
        sv_flat = sv.squeeze(-1) if sv.shape[-1] == 1 else sv.reshape(sv.shape[0], -1)
    elif sv.ndim == 2:
        sv_flat = sv
    else:
